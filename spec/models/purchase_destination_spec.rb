  ...
  describe '発送先情報の保存' do
    ...
    context '内容に問題がある場合' do
      ...
      it 'postal_codeが「3桁ハイフン4桁」の半角文字列でなければ保存できないこと' do
        ...
        @purchase_destination.valid?
        expect(@purchase_destination.errors.full_messages).to include('郵便番号にハイフン(-)を含めて入力してください')
      end
      ...
      it 'phoneが9桁以下では保存できないこと' do
        ...
        @purchase_destination.valid?
        expect(@purchase_destination.errors.full_messages).to include('電話番号は10ケタか11ケタの半角数値で入力してください')
      end
